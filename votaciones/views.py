...
def mostrar_tarjeton(request):
    ...
    # Mostrar
        ...
        id_jornada_corporacion = request.session.pop('id_corporacion', None)
        if id_jornada_corporacion is not None:
...
def mostrar_corporaciones(request, usuario, votantes, jornada):
    ...
    if request.POST and "btnCorporacion" in request.POST:
        ...
        request.session["id_corporacion"] = id_jornada_corporacion
