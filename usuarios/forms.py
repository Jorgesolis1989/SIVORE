...
class FormularioEditarUsuario(forms.Form):
    ...
    esta_activo = forms.BooleanField(initial=True, required=False, widget=forms.CheckboxInput(attrs={'class': 'form-checkbox form-icon'}))

    email = forms.EmailField(
        widget=forms.EmailInput(attrs={'class': 'form-control', 'placeholder': 'Escriba aquí su correo electronico', 'required':'true'}))


class FormularioCargaArchivo(forms.Form):
    cargar = forms.CharField(widget=forms.FileInput(attrs={'required':'true'}))
